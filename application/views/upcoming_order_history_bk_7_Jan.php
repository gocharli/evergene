						...
						<div class="col-lg-12 col-xs-12">
							<?php if($orders){ foreach ($orders as $row) {	?>
								...
								$detail=$this->db->query('select order_details.*,tests.testName,tests.testLogo,tests.testDescription FROM order_details
									...
									WHERE userId='.$this->session_data->userId.' and scheduleDate >= CURDATE() and YEAR(scheduleDate) = '.$year.' and MONTH(scheduleDate)='.$month)->result();
								foreach ($detail as $d){
								?>
												...
												<ul class="clearfix">
													...
													<li>
														<div class="row">
															...
															<span class="detail" style="width: 100%; margin-top: 10px;"><?=date('F d,Y',strtotime($d->scheduleDate))?></span>
