													...
													<div class="col-md-12">
														...
														if (isset($this->session_data->userId) && $this->response['membership_type']->mpId == $row->mpId) {
															if ($this->membership_data->expire == false) {
																...
																<a id="pay_<?php echo $row->mpId; ?>" href="<?php if ($subbb == 'active') echo 'script:;';
																											else echo base_url() . 'memberships/buy_memb/' . $row->mpId; ?>" class="tran3s custom-btn small-btn cart-button btn-pay block hvr-trim-two">BUY NOW</a>
															<?php } ?>

														<?php } elseif (isset($this->session_data->userId)) { ?>
															<input type="hidden" id="plan_price_<?php echo $row->mpId; ?>" value="<?= $row->planAmount - $bonus ?>" />
															<a style="<?php if ($subbb == 'active') echo 'background: #f3eded;pointer-events: none;'; ?>" id="pay_<?php echo $row->mpId; ?>" href="<?php if ($subbb == 'active') echo 'script:;';
																																																else echo base_url() . 'memberships/buy_memb/' . $row->mpId; ?>" class="mt-39 tran3s custom-btn small-btn cart-button btn-pay block hvr-trim-two">BUY NOW</a>
														<?php } else { ?>
		...
		<script>
			function apply_discount() {
				...
				if (code != "") {
					...
					$.post("<?php echo base_url(); ?>memberships/add_coupon", {
						code: code
					}, function(resp) {
						res = JSON.parse(resp);
						console.log(resp);
