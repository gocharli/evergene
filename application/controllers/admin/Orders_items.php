...
class Orders_items extends CI_Controller {
	...
	public function view($detailId=0) {
		...
		//print_r($response['res_unit']); exit;
		
		//$this->data['res_unit'] = $this->data['marker']->tm_unit;
		// End
		...
		//echo '<pre>'; print_r($order_details); exit; // if testResultType = Result 4
	...
	public function update_result3(){
		...
		//$this->db->where("resultId", $resultId)->update("results", array("result3"=>$result3));
	...
	public function change_status() {
		...
		if($type=='Inprogress') {
			...
			if($users->noti_status > 0){
				...
				//$message='Dear '.$users->userFirstName.' '.$users->userLastName.', <p>Your order #'.$order_details->orderId.' is in process. We will let you know when results arrive';			
	...
	public function addTestResult4() {
		...
		if($value==true){
			...
			//for ($i=0; $i < sizeof($result4File); $i++) { 
	...
	public function saveAndPublishResult() {
		...
		//$message='Dear '.$order_details->userFirstName.' '.$order_details->userLastName.', <p>Your test results against order #'.$order_details->orderId.' has been added successfully. Please flollow <a href="'.base_url().'results/view/'.$detailId.'" Link </a> to view details';			
		$this->smtp_email->send('','',$to,$subject,$message);
