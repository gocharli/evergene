...
class Cart extends CI_Controller {
	...
	public  function index() {
		...
		$a=1;
        $new_cart=array();
		$p_type=0;  // to check all paymentType are Discounted Price
        foreach($cart as $row) {
			//echo '<pre>'; print_r($cart); exit;
			// Code added by david
			if($row['options']['paymentType'] != 'Discounted Price'){
				$p_type = 1;
			}
			// Code added by david
			//echo 'options'
    			 ...
    			 if($a <= $toBeChanged) {
                    ...
                    if(trim($row['options']['discountPercentage'])=='Yes') {
					   ...
					   if($row['options']['discount']>0) {
						...
						//$row['price']=$row['options']['orginalPrice']-$row['options']['discount']; // commented by david
						
						// Code added by david
						$testid = $row['id'];	
						$test=$this->db->query('select discountPercentage, discountPrice, discountedPrice from tests WHERE testId='.$testid)->row();
		...
		$this->data['p_type'] = $p_type;  // added by david 

		//$this->cart->destroy();
		//$this->cart->insert($new_cart);
        $this->data['cart_data_new'] = $new_cart; //$new_cart;
	    ...
	    if ($this->membership_data->expire == false) {
		    ...
		    if ($this->form_validation->run() == FALSE) {
			   ...
			   if ($purchaseType != 'Regular') {
				    ...
				    if($uqty>0) {
						//echo $uqty.' second'; exit;
						//Code added by David
						// $cart=$this->cart->contents();
						// $incart = 0;
						// foreach($cart as $row) {
						// 	if($row['options']['pType'] == 'One Time'){
						// 		$incart += $row['qty'];
						// 	}
						// 	//echo $row['options']['pType'];
						// }
						// //echo $incart; exit;
						// if($incart == 0) {
						// 	$uqty = $quantity;
                ...
                if($qty<4)
                 {
                   ...
                   $ref_id=randomString();
                   //$payDebit = $this->input->post("paymentTypeee");
                   $payDebit='No';
                   $pType='Regular';  
                   $directDebit = 'No';
