...
class Memberships extends CI_Controller
{
	...
	public function process()
	{
		...
		// for free membership  // 5 June 2020

		$free_plan = $this->input->post('free_plan');
		$planId = $this->input->post('planId');

		if ($free_plan > 0) {   // 1 = free membership

			$mpId = $this->input->post('planId');
			$plan = $this->db->query('select * from membership_plan where mpId=' . $mpId)->row();
			$mship = $this->db->query('SELECT * FROM memberships WHERE userId=' . $this->session_data->userId)->row();

			$current_period_start = strtotime(date('Y-m-d'));
			$current_period_end = strtotime(date('Y-m-d', strtotime('+1 year')));
			if ($planId == 1) {
				$current_period_end = strtotime(date('Y-m-d', strtotime('+1 month')));
			}
